...
const TOURS = [
  {
    ...
    image: "https://images.unsplash.com/photo-1620736663824-18f7d3a79d54?w=1200&q=80",
    price: 320,
...
export default async function CultureTourPage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
    ...
    <>
      ...
      <section className="relative flex min-h-[60vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-12 pt-32 sm:px-6 lg:px-8">
          <Link href="/culture" className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-white/80 hover:text-white">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            {t("back_to_tours")}
          </Link>
          <span className="inline-flex rounded-full bg-rose-100 px-3 py-1 text-xs font-semibold text-rose-800">{tn("culture")}</span>
          <h1 className="mt-3 font-heading text-3xl font-bold text-white sm:text-4xl md:text-5xl">{tour.title}</h1>
        </div>
      </section>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-100">
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-bold text-rose-700">${tour.price}</span>
                  <span className="text-sm text-gray-500">USD / {t("per_person")}</span>
                </div>
                <Link href="/contact" className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-rose-700 px-6 py-3.5 text-base font-semibold text-white shadow-md hover:opacity-90">
                  {t("book_this_tour")}
                </Link>
              </div>
            </div>
      ...
      <BookingInquiryForm segment="culture" tourSlug={tour.slug} tourTitle={tour.title} />
    </>
  );
}
