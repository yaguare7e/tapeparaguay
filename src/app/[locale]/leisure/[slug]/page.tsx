...
export default async function LeisureTourPage({
  params,
}: {
  params: Promise<{ locale: string; slug: string }>;
}) {
    ...
    <>
      ...
      <section className="relative flex min-h-[60vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-12 pt-32 sm:px-6 lg:px-8">
          <Link href="/leisure" className="mb-4 inline-flex items-center gap-1.5 text-sm font-medium text-white/80 hover:text-white">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            {t("back_to_tours")}
          </Link>
          <span className="inline-flex rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-800">{tn("leisure")}</span>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-2xl bg-white p-6 shadow-lg ring-1 ring-gray-100">
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-bold text-amber-700">${tour.price}</span>
                  <span className="text-sm text-gray-500">USD / {t("per_person")}</span>
                </div>
                <Link href="/contact" className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg bg-amber-700 px-6 py-3.5 text-base font-semibold text-white shadow-md hover:opacity-90">
                  {t("book_this_tour")}
                </Link>
              </div>
            </div>
      ...
      <BookingInquiryForm segment="leisure" tourSlug={tour.slug} tourTitle={tour.title} />
    </>
  );
}
